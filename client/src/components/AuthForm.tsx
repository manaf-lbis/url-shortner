...
//       setLoading(false)
//       setSuccess(
//         type === "login" ? `Welcome back! Signed in as ${email}` : `Account created successfully! Welcome ${email}`,
//       )
//       setEmail("")
//       setPassword("")
//       setConfirmPassword("")
...
//       {/* Submit Button */}
//       <ShinyButton onClick={()=>{}} type="submit" size="md" color="green" disabled={loading} className="w-full mt-6">
...
export default function AuthForm({ type, testCredentials }: AuthFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false)
      setSuccess(type === "login" ? `Welcome back, ${email}!` : `Account created for ${email}`)
      setEmail("")
      setPassword("")
      setConfirmPassword("")
    }, 1500)
